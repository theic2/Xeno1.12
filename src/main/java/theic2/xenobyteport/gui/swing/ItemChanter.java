...
public class ItemChanter extends XenoJFrame implements KeyListener {
    ...
    private JComboBox<String> effectBox, enchBox;
    ...
    public void fillData() {
        for (Enchantment enchantment : Enchantment.REGISTRY) {
            if (enchantment != null) {
                enchBox.addItem(Enchantment.getEnchantmentID(enchantment) + separator + I18n.format(enchantment.getName()) + " (" + enchantment.type.name() + ")");
            }
        }
        for (Potion potion : Potion.REGISTRY) {
            if (potion != null) {
                effectBox.addItem(Potion.getIdFromPotion(potion) + separator + I18n.format(potion.getName()));
            }
        }
    }
    ...
    private boolean checkInputs() {
        ...
        try {
            JsonToNBT.getTagFromJson(nbtArea.getText());
    ...
    public void loadCustomNBT(NBTTagCompound in) {
        String[] display = getDisplayData(editableNBT = in.copy());
    ...
    public void loadCustomNBT(String json) {
        try {
            loadCustomNBT(JsonToNBT.getTagFromJson(json));
        } catch (NBTException e) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (button == accept) {
            if (checkInputs()) {
                try {
                    outNBT = JsonToNBT.getTagFromJson(nbtArea.getText());
                    dispose();
